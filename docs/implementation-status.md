...
| F-15 | Báo cáo, audit log và cấu hình cửa hàng | Đang làm | ReportService.GetShiftReportAsync đã implement đầy đủ (total sales, cash/VNPay revenue, order count, top products, order log). ReportViewModel và ReportView.xaml đã xong với 4 summary card, recent shifts, top products bar chart, order log table. ReportServiceTests đã implement 7 test thật (GetShiftReportAsync: valid shift, shift not found, revenue calculation, cash/VNPay split, top products, order log, empty shift). AuditLogRepository (AddAsync, GetByEntityAsync, GetRecentAsync) và AuditService.LogAsync đã implement. **Dashboard feature mới**: DashboardService, DashboardViewModel (overview, navigation đến 4 sub-dashboard), DashboardView + 4 sub-views (CatalogPromo, Inventory, Report, UserStaff). Còn thiếu: GetSalesReportAsync (NotImplementedException), AuditLogViewModel (skeleton), store configuration. |
...
| SmartPOS.Services | Đang làm | Đã có logic: Auth, Shift, Product, Cart (tax 10%, inactive/stock validation, promotion deduction), Catalog (CRUD + deactivate/reactivate/image + SKU/barcode validation + stock update + two-way sync), Cash Payment, VNPay (tạo URL/QR/poll/cancel/callback), Invoice (tạo hóa đơn theo ngày + preview), Device (fake print + log), InventorySync (sync catalog/stock + deduct/restock + register + sync log), Report (shift report + recent shifts + top products + order log), Audit (log async), Customer (10 method: CRUD + loyalty + list/detail/toggle), Promotion (22 method: validate/apply/approval/create/CRUD/update), Dashboard (overview data). Còn stub: ReturnService (NotImplementedException), ReportService.GetSalesReportAsync. |
| SmartPOS.WPF | Đang làm | **18 ViewModel** đã tạo: LoginViewModel (real), ShiftViewModel (real — clock realtime), SalesViewModel (real — search/barcode/cart/customer/promotion/loyalty/checkout), PaymentViewModel (real — cash + VNPay QR/polling), InvoiceViewModel (real — preview + fake print), CatalogViewModel (real — CRUD + filter/search/deactivate/reactivate/image/stock + inline sync), ReportViewModel (real — shift report + recent shifts + top products + order log), SyncViewModel (real — sync catalog/stock/all), CustomerViewModel (real — search/filter/sort/detail/edit/toggle status/view orders), PromotionViewModel (real — CRUD/search/filter/create popup/toggle active), **DashboardViewModel** (real — overview + navigate to 4 sub-dashboards), **DashboardCatalogPromoViewModel** (real), **DashboardInventoryViewModel** (real), **DashboardReportViewModel** (real), **DashboardUserStaffViewModel** (real). Các màn vẫn placeholder/TODO: ReturnViewModel, AuditLogViewModel, UserManagementViewModel. **18 View** tương ứng. NavigationService map ViewModel → View bằng convention. App.xaml.cs tự động migrate database và seed data khi startup. Hệ thống theme trung tâm trong `Themes/` (13 XAML files). |
