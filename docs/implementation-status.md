...
10. **Theme system**: ✅ Đã hoàn thành — hệ thống theme trung tâm (`Themes/` folder, 13 XAML files) đã triển khai, 9 view đã refactor dùng global `{StaticResource}`. Màu primary đã chuyển sang `#0062FF` (bright blue) theo thiết kế glassmorphism từ `style.css`.
