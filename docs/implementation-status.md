...
| F-15 | Báo cáo, audit log và cấu hình cửa hàng | Đang làm | ReportService.GetShiftReportAsync đã implement đầy đủ (total sales, cash/VNPay revenue, order count, top products, order log). ReportViewModel và ReportView.xaml đã xong với 4 summary card, recent shifts, top products bar chart, order log table. ReportServiceTests đã implement 7 test thật. **Dashboard feature**: DashboardService đã kết nối database thật lấy động TodayRevenue, TotalOrders, NewCustomers, InventoryAlerts, Revenue7Days, TopCategories, RecentTransactions, ActivePromotions. DashboardViewModel, DashboardView + 4 sub-views (CatalogPromo, Inventory, Report, UserStaff) hoạt động tốt. Đã sửa lỗi tranh chấp DB (Race condition) khi khởi tạo Catalog. Còn thiếu: GetSalesReportAsync (NotImplementedException), AuditLogViewModel (skeleton), store configuration. |
...
| SmartPOS.Data | Đang làm | Đã có entity, DbContext, design-time factory, 6 migrations POS (InitialCreate, AddUserContactFields, AddAffectedRowsToSyncLog - đã sửa lỗi thiếu Designer file và apply thành công, AddProductImagePath, AddLoyaltyPointsToOrder, UpdateCustomerStatusAndOrders), DataSeeder sản phẩm mẫu. Các repository đã implement: Auth, Shift, Product, Category, Order, Invoice, Device, DeviceLog, InventorySyncLog, AuditLog, Promotion, Customer. Repository Return còn stub (NotImplementedException). |
| SmartPOS.Services | Đang làm | Đã có logic: Auth, Shift, Product, Cart (tax 10%, inactive/stock validation, promotion deduction), Catalog (CRUD + deactivate/reactivate/image + SKU/barcode validation + stock update + two-way sync), Cash Payment, VNPay (tạo URL/QR/poll/cancel/callback), Invoice (tạo hóa đơn theo ngày + preview), Device (fake print + log), InventorySync (sync catalog/stock + deduct/restock + register + sync log), Report (shift report + recent shifts + top products + order log), Audit (log async), Customer (10 method: CRUD + loyalty + list/detail/toggle), Promotion (22 method: validate/apply/approval/create/CRUD/update), Dashboard (overview data từ database thật). Còn stub: ReturnService (NotImplementedException), ReportService.GetSalesReportAsync. |
| SmartPOS.WPF | Đang làm | **18 ViewModel** đã tạo: LoginViewModel, ShiftViewModel, SalesViewModel, PaymentViewModel, InvoiceViewModel, CustomerViewModel, ReturnViewModel, CatalogViewModel, PromotionViewModel, ReportViewModel, AuditLogViewModel, SyncViewModel, UserManagementViewModel, DashboardViewModel (đã cập nhật tự động gọi LoadData trên Loaded), DashboardCatalogPromoViewModel (đã sửa lỗi Race condition), DashboardInventoryViewModel, DashboardReportViewModel, DashboardUserStaffViewModel. Các màn vẫn placeholder/TODO: ReturnViewModel, AuditLogViewModel, UserManagementViewModel. **18 View** tương ứng. App.xaml đã sửa lỗi StartupUri để dùng DI hoàn toàn. |
| SmartPOS.CallbackApi | Đã xong | Đã có endpoint callback thật cho VNPay, nhận GET/POST, validate signature, gọi PaymentService, bypass SSL localhost khi gọi Inventory API. |
...
| SmartPOS.Tests | Đang làm | Test suite hiện có **51 test thật** + 1 placeholder (ReturnServiceTests chỉ có `Assert.True(true)`). Các test đã implement: AuthServiceTests (7), ShiftServiceTests (5), CartServiceTests (8 test thật - add/update/remove/recalculate/tax/stock/inactive), PaymentServiceTests (11 bao gồm VNPay), InvoiceServiceTests (4), DeviceServiceTests (3), InventorySyncServiceTests (6), **ReportServiceTests (7 — GetShiftReportAsync valid/not found/revenue/cash-vnpay split/top products/order log/empty)**. PromotionServiceTests đã có test thật. Cần bổ sung: test thật cho Return, và test cho Catalog/Audit. |
...
3. **Báo cáo doanh thu** (F-15): Implement `ReportService.GetSalesReportAsync`.
4. **Audit log viewer** (F-15): Implement `AuditLogViewModel` để hiển thị audit logs.
5. **Quản lý user**: Implement `UserManagementViewModel` (tạo tài khoản mới khi giảng viên yêu cầu).
