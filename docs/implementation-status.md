...
| SmartPOS.Services | Đang làm | Đã có logic: Auth, Shift, Product, Cart (tax 10%, inactive/stock validation, promotion deduction), Catalog (CRUD + deactivate/reactivate/image + SKU/barcode validation + stock update + two-way sync), Cash Payment, VNPay (tạo URL/QR/poll/cancel/callback), Invoice (tạo hóa đơn theo ngày + preview), Device (fake print + log), InventorySync (sync catalog/stock + deduct/restock + register + sync log), Report (shift report + sales report + recent shifts + top products + order log), Audit (log async), Customer (10 method: CRUD + loyalty + list/detail/toggle), Promotion (22 method: validate/apply/approval/create/CRUD/update), Dashboard (overview data từ database thật). Còn stub: ReturnService (NotImplementedException). |
...
| SmartPOS.Tests | Đang làm | Test suite hiện có **54 test thật** + 2 placeholders (ReturnServiceTests và PromotionServiceTests chỉ có `Assert.True(true)`). Các test đã implement: AuthServiceTests (7), ShiftServiceTests (5), CartServiceTests (8 test thật - add/update/remove/recalculate/tax/stock/inactive), PaymentServiceTests (11 bao gồm VNPay), InvoiceServiceTests (4), DeviceServiceTests (3), InventorySyncServiceTests (6), **ReportServiceTests (12 — GetShiftReportAsync và GetSalesReportAsync đầy đủ)**. Cần bổ sung: test thật cho Return, và test cho Catalog/Audit/Promotion. |
